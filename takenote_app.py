...
#         # Try to save the raw data as a single file
#         if data_str.strip():
...
#     sock.settimeout(10.0)  # Timeout after 10 seconds of silence
#     buffer = bytearray()
...
#             # Check if the final transfer marker is already received
...
# def test_individual():
#     """Test individual characters from the text"""
#     text_content = txt1.get("1.0", tk.END)
#     count = 0
    
#     print("Testing individual characters:")
#     for char in text_content[:-1]:  # Exclude the last newline added by Tkinter
#         print(f"Character {count}: '{char}' (ASCII: {ord(char)})")
#         count += 1
        
#         # Add processing logic here for each character
#         if char.isalpha():
#             if char.islower():
#                 braille_pattern = alplow_brl(char)
#             else:
#                 braille_pattern = alpup_brl(char)
#         elif char.isdigit():
#             braille_pattern = num_brl(char)
#         else:
#             braille_pattern = alplow_brl(char)  # Handle special characters
            
#         # Process braille pattern (add your hardware control logic here)
        
#     print(f"Total characters processed: {count}")
...
#!/usr/bin/env python3
"""
TakeNote - Braille Text Analysis Tool with Automated Testing
Integrated version combining GUI analysis and hardware testing
"""
...
def transfer_files():
    ...
    try:
        ...
        # Automatically display the latest file
        if saved_files:
            latest_file = max(saved_files, key=os.path.getctime)  # Get most recently created file
            print(f"📄 Latest file: {latest_file}")
            
            # If you have a function to display/open the file, call it here
            # For example:
            display_file(latest_file)  # You'll need to implement this function
            # Or update a label/text widget with the file content
...
def display_file(filepath):
    """Display the content of the transferred file"""
    try:
        # If it's a text file, read and display it
        if filepath.endswith('.txt'):
            with open(filepath, 'r', encoding='utf-8') as f:
                content = f.read()
                # Update your UI to show this content
                # For example, if you have a Text widget:
        ...
        # If it's an image, display it
            ...
            # Open image with default viewer or display in your app
            ...
            # For other file types, just open with default application
