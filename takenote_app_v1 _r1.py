...
braille_map = {
    ...
    '"': "5",           # Quotation mark
    "-": "3,6",         # Hyphen
    "(": "2,3,5,6",     # Opening parenthesis
    ")": "2,3,5,6",     # Closing parenthesis
    "/": "3,4",         # Slash
    "@": "4,1",         # At sign
    "#": "3,4,5,6",     # Number sign
    "$": "4,2,3,4",     # Dollar sign
    "%": "4,3,5,6",     # Percent sign
    "&": "4,1,2,3,4,6", # Ampersand
    "*": "5,3,5",       # Asterisk
    "+": "5,2,3,5",     # Plus sign
    "=": "5,2,3,5,6",   # Equals sign
    " ": "0"            # Space
}
...
def compare_user_input():
    ...
    if total_errors == 0:
        result_text.insert(tk.END, "Perfect typing! No errors found.\n", "success")
    ...
    header = "{:<15} | {:<15} | {:<35}\n".format(
        "Original", "Typed", "Error description"
    )
    ...
    header2 = "{:<20} | {:<10} | {:<10} | {:<10} | {:<5}\n".format(
        "Error", "Correct", "Wrong", "Missed", "freq"
    )
...
btn_test_all = tk.Button(ref_buttons_frame, text="TEST", command=lambda: test_individual(txt1, status_label),
